...
/*
        ...
        if (Objects.nonNull(filterRequest.getFilteredUsername()) && filterRequest.getFilteredUsername() != "")
            uPSB.with(new SearchCriteria("username", filterRequest.getFilteredUsername(), SearchOperation.LIKE, Datatype.STRING));
...
/*
    ...
    public Page<User> viewAllRecord(UserMgmtFilter details, Integer pageNo, Integer pageSize, String operation) {
        try {
            ...
            if (operation.equals("Export")) {
                ...
                orderColumn = "Created On".equalsIgnoreCase(details.getOrderColumnName()) ? "createdOn" : "Modified On".equalsIgnoreCase(details.getOrderColumnName()) ? "modifiedOn" : "User ID".equalsIgnoreCase(details.getOrderColumnName()) ? "username" : "Email".equalsIgnoreCase(details.getOrderColumnName()) ? "userProfile.email" : "Phone No.".equalsIgnoreCase(details.getOrderColumnName()) ? "userProfile.phoneNo" : "User Type".equalsIgnoreCase(details.getOrderColumnName()) ? "usertype.usertypeName" : "modifiedOn";
            }
            ...
            userService.saveUserTrail(details.getUserId(), details.getUsername(), details.getUserType(), details.getUserTypeId(), Features.User_Management, operationType, details.getFeatureId(), details.getPublicIp(), details.getBrowser());
    ...
    public GenricResponse updateUser(UserDetails details) {
        ...
        // RequestHeaders(details.getUserAgent(),details.getPublicIp(),details.getUsername());
        // headerService.saveRequestHeader(header);
        userService.saveUserTrail(details.getUserId(), details.getUsername(), details.getViewUserType(), details.getUsertypeId(), Features.User_Management, SubFeatures.UPDATE, details.getFeatureId(), details.getPublicIp(), details.getBrowser());
    ...
    public GenricResponse deleteById(AllRequest details) {
        try {
            ...
            // RequestHeaders(details.getUserAgent(),details.getPublicIp(),details.getUsername());
            // headerService.saveRequestHeader(header);
            userService.saveUserTrail(details.getUserId(), details.getUsername(), details.getUserType(), details.getUserTypeId(), Features.User_Management, SubFeatures.DELETE, details.getFeatureId(), details.getPublicIp(), details.getBrowser());
    ...
    public GenricResponse viewById(AllRequest request) {
        ...
        // headerService.saveRequestHeader(header);
        userService.saveUserTrail(request.getUserId(), request.getUsername(), request.getUserType(), request.getUserTypeId(), Features.User_Management, "View Profile", request.getFeatureId(), request.getPublicIp(), request.getBrowser());
        User output = new User();
        try {
        ...
        if (output != null) {
            ...
            UserDetails details = new UserDetails(output.getUserProfile().getFirstName(), output.getUserProfile().getMiddleName(), output.getUserProfile().getLastName(), output.getUserProfile().getPhoneNo(), output.getUserProfile().getEmail(), output.getUsername(), request.getDataId(), output.getRemark(), output.getApprovedBy(), output.getCurrentStatus());
